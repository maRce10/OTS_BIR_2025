...
Digitizing implies discretizing, which requires some sort of regular sampling. Sampling frequency refers to how many samples of the pressure level of the environment are taken per second. A 440 Hz sine wave would have around 100 samples per cycle. This plot shows 2 cycles of a 440 Hz sine wave sampled (vertical dotted lines) at 4400 Hz (10 times its frequency):
...
# generate sine wave
sn.wv <- tuneR::sine(freq = 440, duration = 200, xunit = "samples", samp.rate = 44000)
...
Sampling should be good enough so the regularity of the sine wave can be reconstructed from the sampled data. Low sampling frequencies of a high frequency sine wave might not be able to provide enough information. For instance, the same 440 Hz sine wave sampled at 2200 Hz (5 times its frequency) looks like this:
...
points(y = sn.wv@left[seq(0, length(sn.wv), 20)], x = seq(0, duration(sn.wv), length.out = length(sn.wv))[seq(0, length(sn.wv), 20)], col = "orange", pch = 20, cex = 2)
...
par(mar = c(4, 4, 1, 1), bg = "#d1e7dd")
# generate sine wave
sn.wv <- tuneR::sine(freq = 400, duration = 200, xunit = "samples", samp.rate = 44000)
sn.wv2 <- tuneR::sine(freq = 400, duration = 4, xunit = "samples", samp.rate = 880)
