...
### Optional features (if `harmonicity = TRUE`)
...
<img src="images/DTW.png" alt="viewSpec" width="100%"/>
...
<img src="images/Phae.long1.wav-1-dfDTW.jpeg" alt="dfdtw" width="80%"/>
...
### Features at higher levels of organization
...
## import Phaethornis-eurynome data
# annotations

pha_eur_anns <- imp_raven(
  files = "Phaethornis-eurynome-15607.Table.1.selections.txt",
  path = "./examples",
  warbler.format = TRUE,
  all.data = TRUE
)
...
# acoustic data
pha_eur_wav <- read_sound_file("Phaethornis-eurynome-15607.wav", path = "./examples", from = 4.1, to = 6.4)

spectro(pha_eur_wav, scale = FALSE, flim = c(1, 13), collevels = seq(-120, 0, 1), wl = 200, ovlp = 0, palette = mako, grid = FALSE)

sub_anns <- pha_eur_anns[5:10, ]

sub_anns$start <- sub_anns$start - 4.1
sub_anns$end <- sub_anns$end - 4.1

song_anns <- song_analysis(sub_anns, song_colm = "song")

sub_anns$start <- sub_anns$start + 0.03
sub_anns$end <- sub_anns$end - 0.03


for (i in seq_len(nrow(sub_anns))) {
      lines(x = (sub_anns[i, c("start", "end")]), y = rep(2, 2), col = "#CF4446FF", lwd = 10, lend = 2)
text(x = (sub_anns[i, "start"] + sub_anns[i, "end"]) / 2, y = 2.2, labels = sub_anns[i, "elm"], col = "black", cex = 1)
      }

for (i in seq_len(nrow(song_anns))) {
      lines(x = (song_anns[i, c("start", "end")]), y = rep(12, 2), col = "#F7D03CFF", lwd = 10, lend = 2)
  text(x = (song_anns[i, "start"] + song_anns[i, "end"]) / 2, y = 12, labels = paste("song", song_anns[i, "song"] - 2), col = "black", cex = 1)

    }

# pha_eur_est <- selection_table(pha_eur_anns,
#                                extended = TRUE,
#                                path = "./examples",
#                                by.song = "song")
# 
# saveRDS(file = "./examples/pha_eur_est.RDS", object = pha_eur_est)
...
```{r, eval= TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height= 2}

# read  the first 8 rows of the extended selection table
sth_est <- readRDS(file = "./examples/pha_eur_est.RDS")[1:8, ]
