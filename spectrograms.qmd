...
.specPlot <- function(wave, frq.lim, wl, ovlp, wn, main, ...) {
    fspec <- spectro(wave = clip, wl = wl, ovlp = ovlp, 
                     wn = wn, plot = FALSE)
    t.bins <- fspec$time
    ...
    frq.step <- frq.bins[2] - frq.bins[1]
      ...
      axis(2, at = frq.bin.ticks/frq.step, labels = frq.bin.ticks, 
           las = 1)
     t.bin.ticks <- pretty(seq(par("usr")[1], par("usr")[2], 10), n = 4) 
      axis(1, at = t.bin.ticks, labels = round(t.bin.ticks * duration(clip) / par("usr")[2], 2))
...
plot(h1[1:75], type = "l", col = "#38AAACFF", xlab = "Time (samples)", ylab = "Amplitude (no units)")

# frec 2
h2 <- 10 * cos(2 * w0 * t)

plot(h2[1:75], type = "l", col = "#38AAACFF", xlab = "Time (samples)", ylab = "Amplitude (no units)")

# frec 3
h3 <- 15 * sin(3 * w0 * t)

plot(h3[1:75], type = "l", col = "#38AAACFF", xlab = "Time (samples)", ylab = "Amplitude (no units)")
...
plot(H0[1:75], type = "l", col = "#38AAACFF", xlab = "Time (samples)", ylab = "Amplitude (no units)")
...
plot(fspc[1:(length(fspc) / 2)], type="h", col="#38AAACFF",
xlab="Frecuency (Hz)",
ylab="Amplitude (no units)")
...
plot(fspc[1:(length(fspc) / 2)], 1:(length(fspc) / 2), type="l", col="#38AAACFF", ylab="Frequency (Hz)",
...
## From Fourier transforms to spectrograms
...
monitoR::viewSpec(clip = s.wav,  ovlp = 95, main = NA, frq.lim = c(0, 3))
  ...
  for(x in 2:length(sq))
{
  ...
  plot(fspc1[1:(length(fspc1) / 2)], 1:(length(fspc1) / 2), type="l", col="#38AAACFF", ylab="Frequency (Hz)", xlab="Intensity", lwd = 3, ylim = c(20, 220))
  ...
  par(mar = c(4, 0, 1, 1))
  # spectro(s, palette = reverse.gray.colors.1, scale = FALSE, grid = FALSE, f = f, flim = c(0.2, 3),  flab = NULL, collevels = seq(-100, 0 , 5), wl = 250)
  
  .specPlot(s, wl = 800, ovlp = 0, wn = "hanning", frq.lim = c(0, 3))
  
    usr <- par("usr")
  sq.spc <- seq(usr[1], usr[2], length.out = num.segmts + 1)

  
  for (e in 2:length(sq))
  rect(xleft = sq.spc[e - 1], xright = sq.spc[e], ytop = 400, ybottom = 0, col = adjustcolor("white", 0), border = "gray")
    
  rect(xleft = sq.spc[x - 1], xright = sq.spc[x], ytop = 400, ybottom = 0, col = adjustcolor("#38AAACFF", 0.1), border = NA)
  
  text(x = mean(sq.spc[x - 1], sq.spc[x]), y = 250, labels = (1:num.segmts)[x - 1], cex = 3, pos = 4)
  
  },
movie.name =  "periodogram_spectro.gif", interval = 0.5, ani.width = 480 * 1.5)
...
# plot(s[1:1000], type = "l", col = "#38AAACFF", xlab = "Time (samples)", ylab = "Amplitude (no units)")
# 
# plot(s[7000:8000], type = "l", col = "#38AAACFF", xlab = "Time (samples)", ylab = "Amplitude (no units)")
# 
# 
# plot(s, type = "l", col = "#38AAACFF", xlab = "Time (samples)", ylab = "Amplitude (no units)")
...
for(x in 2:length(sq.spc))
  rect(xleft = sq.spc[x - 1], xright = sq.spc[x], ytop = (usr[4] - usr[1]) / 2, ybottom = usr[3], col = adjustcolor("#38AAACFF", 0.1), border = "#38AAACFF")
...
for(x in 2:(length(sq.spc) - 1))
  rect(xleft = sq.spc[x - 1], xright = sq.spc[x], ybottom = (usr[4] - usr[1]) / 3, ytop = (usr[4] - usr[1]) / 8 * 7, col = adjustcolor("#38AAACFF", 0.1), border = "#38AAACFF")
...
for(x in 2:length(sq.spc))
  rect(xleft = sq.spc[x - 1], xright = sq.spc[x], ytop = usr[4], ybottom = usr[3], col = adjustcolor("#38AAACFF", 0.1), border = "#38AAACFF")
...
saveGIF(for(i in c(seq(0, 99, by = 5), rep(99, 15)))
{
  ...
  for(x in 2:length(sq.spc))
    rect(xleft = sq.spc[x - 1], xright = sq.spc[x], ytop = usr[4], ybottom = usr[4] * 3/4, col = adjustcolor("#38AAACFF", 0.1), border = adjustcolor("#38AAACFF", 1 - (i / 100)))
...
plot(res$res.t, res$res.f, type =  "l", col = "#38AAACFF", lwd = 4, xlab = "Time window size (s)", ylab = "Frequency bin size (Hz)")
...
custom_pal <- colorRampPalette( c("#366A9FFF", "#366A9FFF", viridis::mako(10)[5:10]))
...
<iframe width="100%" src="images/nightingale_wren.mp4" allowtransparency="false" style="background: #FFFFFF;" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen>
