...
::: {.alert .alert-warning .unnumbered}

#### Handling missing sound file name columns

Raven-generated .txt annotation tables sometimes lack a column specifying the associated sound file names, particularly when exported from Raven Lite. To address this, the `name.from.file` argument allows users to extract sound file names directly from the .txt filename, assuming the file follows Raven's default naming convention (e.g., SOUND_FILE_NAME.Table.1.selections.txt). When enabling this feature (`name.from.file = TRUE`), users must also specify `ext.case` to indicate the case of the sound file extension (e.g., "lower" for .wav or "upper" for .WAV), ensuring correct file matching. 
